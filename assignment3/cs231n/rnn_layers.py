...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  ##############################################################################
  # Vanilla RNN from Lecture 10 - 17
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  # Initialize cache
  cache = []
  # Transpose x putting T to the front (T,N,H)
  x = x.transpose(1, 0, 2)
  h = np.zeros((T, N, H))
  h[-1] = h0
  for t in xrange(T):
  ...
  # Detranspose the hidden state vector (N,T,H)
  h = h.transpose(1, 0, 2)
...
def rnn_backward(dh, cache):
  ...
  # Same as FP, transpose h for calculations
...
def word_embedding_forward(x, W):
  ...
  ##############################################################################
  assert np.all(x >= 0),"Element of x < 0"
  #Getting the sequence's vector from W
  out = W[x, :]
  cache = x, W
  ##############################################################################
  #                               END OF YOUR CODE                             #
...
def word_embedding_backward(dout, cache):
  ...
  x, W = cache
  dW = np.zeros_like(W)
  #Add dout w.r.t position of x into dW
  np.add.at(dW, x, dout)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
