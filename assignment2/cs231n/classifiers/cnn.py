...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    P = (filter_size - 1) / 2
    out_H = (H + 2 * P - filter_h) / S + 1
    out_W = (W + 2 * P - filter_w) / S + 1
    #Conv layer setup
    ...
    pool_W = 2
    pool_S = 2
    out_pool_H = (out_H - pool_H) / pool_S + 1
    out_pool_W = (out_W - pool_W) / pool_S + 1
    #Hidden layer setup
    W2 = weight_scale * np.random.randn(F * out_pool_H * out_pool_W, hidden_dim)
  ...
  def loss(self, X, y=None):
    ...
    #Output shape
    N, F, out_H, out_W = conv_layer.shape
    #FP for Hidden layer
    #convert into im2col
    x2 = conv_layer.reshape((N, F * out_H * out_W))
    hidden_layer, cache_hidden_layer = affine_relu_forward(x2, W2, b2)
    N, hidden_dim = hidden_layer.shape
    #FP for output layer
    ...
    #calculate loss for each set of weight, add them up for regularization loss
    ...
    dW2 += self.reg * W2
    #BP Conv layer
    dx2 = dx2.reshape(N, F, out_H, out_W)
